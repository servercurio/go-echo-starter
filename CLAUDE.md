...
- **Richer OpenAPI metadata**: the spec generator (`internal/openapi/spec.go`) is intentionally minimal — only path/method/tag/single-200-response per operation. To add request bodies, response schemas, query/header parameters, or richer responses, extend the `router.Endpoint` interface (and its `std/endpoint` implementation) to carry the new metadata, then read it inside `openapi.walk` when emitting `Operation` values. Don't add per-route annotations parsed at runtime — the AST-driven path stays code-only.
- **Release behaviour change**: edit `.releaserc.json` for semantic-release plugin config (commit-analyzer rules, release notes preset, exec hooks, branch channels, GitHub asset list). Cross-check that any new binary added to `bin/` is also listed under `@semantic-release/github` `assets` so it ships with the release.
