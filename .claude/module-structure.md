...
- `.github/workflows/` — CI workflows. PR Formatting and PR Checks (200-series flow workflows) call the reusable code-compiles and unit-test workflows (800-series). Releases run via `100-user-deploy-release-artifact.yaml` (manual `workflow_dispatch`) which delegates to the reusable `800-call-semantic-release.yaml`; semantic-release configuration lives in `.releaserc.json` at repo root. Naming follows the convention documented in `.github/workflows/docs/naming-standards.md`.
